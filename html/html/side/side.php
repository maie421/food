...
<? include "../lib/top_menu.php";?>
  ...
  <div class="menu_banner"><img src="/img/menu/banner3.JPG" id="menu_banner"></div>
<div id="main_main">
<span id="full">상품 0개</span>
<div id="line">
  <ul id="line_list">
    <li><a href="side.php?select=ma">판매인기순</a></li>
    <li><a href="side.php?select=low">낮은가격순</a></li>
    <li><a href="side.php?select=High">높은가격순</a></li>
    <li><a href="side.php?select=day">등록일순</li>
  </ul>
</div>
<?php
  ...
  $scale=10;
  if($select=="ma")
    $sql="select *from side order by hit desc";
	else if($select=="low")
		$sql="select *from side order by price_result";
	else if($select=="High")
		$sql="select *from side order by price_result desc";
  else
...
<div class="number_page_center">
  ...
  <?php for($i=1;$i<=$page;$i++){?>
    <li id="number_page"><a href="side.php?page=<?=$i?>"><?=$i?></a></li>
  <?}?>
  </ul>
</div>
</div><!--main_main-->
