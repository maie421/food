...
<?php
  ...
  //get 방식으로 찾기 기능
  if($find=="s"){//제목
  $sql="select * from faq where subject like '%{$search_text}%'";
  }
  else if($find=="t"){//내용
  $sql="select *from faq where content like '%{$search_text}%'";
  }
  else if($find=="i"){//작성자
  $sql="select *from faq where name like '%{$search_text}%'";
  }
  else {
  ...
  $total_recored=mysql_num_rows($result);//전체 글
  ...
  <div id="notice_main">
    ...
    <table>
      ...
      <tr>
        ...
        <td class="notice_title"><a href="write_in.php?num=<?=$notice_num?>&page=<?=$page?>&mod=faq"><?=$notice_subject?></a></td>
    ...
    <a href="write.php?mod=faq"><div class="write_button">글쓰기</div></a>
    <form method="get">
    <div id="search">
